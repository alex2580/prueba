...
async function crear(req, res, next) {
  try {
    ...
    const oferente = await queryOne('SELECT mp_access_token, verificado FROM usuarios WHERE id = ?', [req.user.id]);
    if (!oferente?.verificado) {
      return res.status(403).json({ error: 'Necesitás verificar tu identidad en tu perfil antes de publicar un espacio' });
    }
