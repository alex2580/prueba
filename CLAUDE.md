...
> **Pendiente (26 ago 2026):** `add-comision-pct.js` — agrega `comision_pct` (DECIMAL, default 15.00) a `usuarios` y `comision_pct_aplicado` a `reservas`. Reemplaza el mecanismo de `early_adopter` (0% automático por 3 meses al registrarse desde la waitlist, ya dado de baja en `middleware/auth.js`): el % ahora se gestiona a mano por usuario desde el panel admin (tab "💰 Comisiones"). La migración hace backfill de `comision_pct = 0` para quienes tenían `early_adopter` vigente al momento de correrla, para no cortarles la promo ya otorgada. Guille debe correr: `node backend/src/db/add-comision-pct.js`.
...
> **Pendiente (26 ago 2026):** `add-didit-verificacion.js` — agrega `didit_session_id`/`verificado_at` a `usuarios` (reusa la columna `verificado` que ya existía sin uso automático). Guille debe correr: `node backend/src/db/add-didit-verificacion.js`. Además Alejandro tiene que crear cuenta en Didit, configurar un workflow (ID Verification + Liveness + Face Match) y cargar 4 env vars en el VPS: `DIDIT_API_KEY`, `DIDIT_WEBHOOK_SECRET`, `DIDIT_WORKFLOW_ID`, `DIDIT_CALLBACK_URL=https://todasmiscosas.com/panel?didit=return` — y apuntar el webhook de Didit a `https://todasmiscosas.com/api/didit/webhook`.
...
> **Pendiente (4 sep 2026):** `add-qr-reserva.js` — agrega columna `qr_token` (único) a `reservas` y genera el token para las reservas existentes. Necesaria para el check-in por QR (el proveedor escanea el QR del cliente para liberar el escrow, alternativa al botón de "confirmar acceso" — ver decisión de arquitectura más abajo). Guille debe correr: `node backend/src/db/add-qr-reserva.js`
...
- **Liberación del escrow por tres caminos (4 sep 2026)**: además del botón de "confirmar acceso" del cliente (`POST /reservas/:id/confirmar-acceso`) y el cron de auto-liberación a las 48hs (`jobs/escrow.js`), el proveedor puede escanear el QR de la reserva (`POST /reservas/checkin/:token`, página `/checkin/[token]`) — mismo mecanismo que el check-in de tickets en TME. Los tres conviven (lo que ocurra primero libera el escrow); `confirmarAcceso()` y `checkinPorQR()` comparten la liberación real (ledger + intento de payout automático vía `transferirDinero()` + emails) en `liberarEscrow()` dentro de `reservasController.js`, para no duplicar esa lógica dos veces en el mismo archivo (el cron de `jobs/escrow.js` sí la mantiene duplicada aparte, por vivir en otro módulo). El QR (`reservas.qr_token`, generado al crear la reserva) se muestra en `EstadoReserva.tsx` solo mientras la reserva está `pagada` y sin liberar.
- **Pago a proveedores: por lote bancario, no por API (4 sep 2026)** — se descartó automatizar la transferencia vía ningún proveedor externo (Mercado Pago Money Out y `marketplace_fee`+captura diferida, Talo, BIND, dLocal, PayU, Ualá Bis: ninguno resuelve "tarjeta + custodia + split variable" para una plataforma chica en Argentina sin sumar un vendedor nuevo o cambiar el método de cobro — investigación completa en memoria, `project_tmc_pago_directo_proveedor.md`). En vez de eso, `GET /api/admin/payouts` (tab "📊 Movimientos" del panel admin, bloque "🏦 Pagos pendientes a proveedores") lista las reservas con `escrow_liberado=1` y sin payout exitoso, `GET /api/admin/payouts/export` arma un CSV agrupado por proveedor (un solo pago aunque tenga varias reservas liberadas) listo para subir al pago por lote del home banking, y `POST /api/admin/payouts/marcar-transferido` confirma a mano después de subirlo. El intento de `transferirDinero()` vía OAuth sigue corriendo antes que esto (en `liberarEscrow()`) pero siempre falla (401, endpoint ya descartado) — por eso casi todas las reservas liberadas terminan apareciendo acá. No se tocó ese intento fallido: falla en silencio y de forma segura, no hace falta apurarse a limpiarlo.
...
- **Comisión (26 ago 2026, configurable por usuario)**: `usuarios.comision_pct` (default 15.00) reemplaza la constante fija que existía antes — el admin la gestiona por proveedor desde el tab "💰 Comisiones" del panel (`GET/PATCH /api/admin/comisiones` y `/api/admin/usuarios/:id/comision`). Se fija en la reserva al pagar (`reservas.comision_pct_aplicado`, seteado en `pagosController._procesarPagada`) para que un cambio de % posterior no afecte reservas ya pagadas — `ledgerService.registrarLiberacion` y el email `sendEscrowRetenidoOferente` usan ese valor fijado, no el `comision_pct` actual del usuario. `frontend/lib/utils.ts` (`COMISION_TMC`/`netoOferente()`) sigue mostrando una estimación fija del 15% en las pantallas de publicar/reservar — no se conectó al valor real por proveedor, así que puede no coincidir exactamente para cuentas con % ajustado a mano.
- **Primeros 50 proveedores de la waitlist → 0% automático (27 ago 2026)**: reemplaza al viejo mecanismo de `early_adopter` (dado de baja el mismo día, ver arriba) con la misma idea pero anclada a `comision_pct`. `middleware/auth.js`, al crear la fila de `usuarios` en el primer login, busca el email en `waitlist` (`tipo='proveedor'`) y calcula su posición contando cuántas filas de proveedor tienen `created_at` menor o igual a la suya — si esa posición es ≤ `CUPOS_0_COMISION_WAITLIST` (50, mismo número que `CUPOS_0_COMISION` en `frontend/app/[locale]/page.tsx`), setea `comision_pct = 0`. El ranking es por orden de anotación en la waitlist, no por orden de creación de cuenta. El banner de la home (`GET /api/waitlist/contador`) muestra cuántos cupos quedan, contando proveedores anotados sobre el mismo tope de 50.
- **Verificación de identidad biométrica (26 ago 2026)** — antes de publicar un espacio, el proveedor tiene que verificar su identidad con Didit (`diditService.js`/`diditController.js`, ruta `/api/didit`): DNI + selfie con prueba de vida comparados contra RENAPER, corre 100% en la infraestructura de Didit — TMC nunca guarda fotos ni datos del documento, solo el resultado (`usuarios.verificado`/`verificado_at`/`didit_session_id`). Gate en `espaciosController.crear`, mismo patrón que el gate de Mercado Pago. **Límite legal deliberado:** esto es verificación de identidad, no antecedentes penales — el art. 7 de la Ley 25.326 reserva el tratamiento de antecedentes penales a autoridades públicas, ninguna empresa privada puede consultarlos; si se quiere algo relacionado, la única vía legal es que el propio usuario suba voluntariamente su Certificado de Antecedentes Penales (lo pide él mismo en mi.argentina.gob.ar), nunca una consulta que haga TMC.
