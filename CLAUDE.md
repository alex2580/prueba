...
> **Pendiente (26 ago 2026):** `add-payout-automatico.js` — agrega `payout_estado`/`payout_mp_id`/`payout_error` a `reservas`, parte de la transferencia automática al proveedor al liberar el depósito de garantía (ver "Pago directo al proveedor" más abajo). Guille debe correr: `node backend/src/db/add-payout-automatico.js`. **Además**, antes de confiar esto en producción hay que confirmar con Mercado Pago que la app de TMC tiene habilitado el scope `money_transfer` (acción de Alejandro en el dashboard de developers de MP, no de código) — sin eso, `transferirDinero()` va a fallar siempre y todo cae al aviso manual de siempre (sin romper nada, pero sin automatizar tampoco).
...
- **Pago directo al proveedor (26 ago 2026)** — el cobro sigue entrando y quedando retenido en la cuenta central de MP exactamente igual que siempre (custodia real, sin cambios — a diferencia de TME, acá NO se usa `marketplace_fee`/OAuth Connect porque ese mecanismo liquida solo al aprobarse el pago y no soporta "retener y liberar después"). Lo que cambia es el momento de la liberación (`confirmarAcceso` en `reservasController.js` y el cron de `jobs/escrow.js`): en vez de solo mandar un email al admin para que transfiera a mano, `mercadopagoService.transferirDinero()` intenta la transferencia automática al alias de MP declarado por el proveedor (reusa la columna `usuarios.cbu_alias`, ahora obligatoria para publicar un espacio — ver `espaciosController.crear` y `pagosController.crearPreferencia`). Si la transferencia falla (scope no habilitado, alias inválido, etc.) cae al aviso manual de siempre, sin romper nada — `reservas.payout_estado` queda en `'fallido'` con el motivo en `payout_error`. La forma exacta del request a `/v1/payments` (`operation_type: 'money_transfer'`) no está confirmada al 100% contra documentación oficial — probar con un monto simbólico antes de confiar en ella para liberaciones reales.
